...
//        message.setUid("UID_v6EV1w6wfxypQDukydGumHjL9ALz");
//        message.setUrl("http://49.233.73.127:1263/#/home");//可选参数
...
//        log.info("消息发送："+result);
        ...
        if (BaseCallBackReq.ACTION_APP_SUBSCRIBE.equalsIgnoreCase(callBackReq.getAction())) {
            ...
            if (!StringUtils.isEmpty(appSubscribeBean.getUid())) {
                ...
                message.setUrl("http://www.xiaoming100.club:1264/#/home");//可选参数
                Result<List<MessageResult>> result = WxPusher.send(message);
                log.info("消息发送：" + result);
